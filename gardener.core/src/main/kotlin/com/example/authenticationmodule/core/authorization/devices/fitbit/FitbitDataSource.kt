...
import com.example.authenticationmodule.core.common.util.uri.HttpMethod
import com.example.authenticationmodule.core.common.util.uri.Uri
import com.example.authenticationmodule.core.authorization.datasource.oauth2.*
import com.example.authenticationmodule.core.common.events.datacollection.DataCollectionPreparationEvent
...
class FitbitDataSource(
  ...
  enum class Scopes(val key: String) {
    ...
    companion object {
      fun isValid(scope: String): Boolean {
        return  values().firstOrNull { it.key == scope } != null
      }
    }
  }
  ...
  override fun constructScopeStringsForAuthorizationUrl(scopes: List<String>?): String {
    ...
    return Scopes.values().map { it.key }.toList().joinToString(" ")
  }
  ...
  override fun assembleDataCollectionUri(
    ...
    rawPing: JsonNode
      ...
      val uriString: String = when (dataType) {
        FitbitDataCollectionType.ACTIVITIES -> "${clientSettings.dataUrl}/${dataType.version}/user/-/${dataType.getEndpoint()}/date/${rawPing.get(AP_DATE_KEY).textValue()}.json"
        FitbitDataCollectionType.HEART_RATE -> "${clientSettings.dataUrl}/${dataType.version}/user/-/${dataType.getEndpoint()}/date/today/1d.json"
        FitbitDataCollectionType.BODY -> "${clientSettings.dataUrl}/${dataType.version}/user/-/${dataType.getEndpoint()}/date/${rawPing.get(AP_DATE_KEY).textValue()}.json"
        FitbitDataCollectionType.SLEEP -> "${clientSettings.dataUrl}/${dataType.version}/user/-/${dataType.getEndpoint()}/date/${rawPing.get(AP_DATE_KEY).textValue()}.json"
        FitbitDataCollectionType.FOOD -> "${clientSettings.dataUrl}/${dataType.version}/user/-/${dataType.getEndpoint()}/date/${rawPing.get(AP_DATE_KEY).textValue()}.json"
      }
  ...
  override fun getDataCollectionPreparationEventFromPing(notification: String): List<DataCollectionPreparationEvent> {
    ...
    try {
      ...
      notificationNode.forEach { ping ->
        val dataType = FitbitDataCollectionType.from(ping.get(AP_DATA_TYPE_KEY).textValue()) ?:
