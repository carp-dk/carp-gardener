...
import com.example.authenticationmodule.core.authorization.datasource.DataSource
...
abstract class OAuth1DataSource(
  ...
  init {
    ...
    eventBus.subscribe(
      ...
      handler = { event: OAuth1Event.AuthorizedTokenAcquired -> acquireAccessToken(event) } as (DataSourceEvent) -> Unit
    )
  }
  ...
  protected fun acquireAccessToken(event: OAuth1Event.AuthorizedTokenAcquired) {
    ...
    val accessParams: OAuth1AccessParams = authorizationOperator.acquireAccessToken(
      userId = state.userId,
      dataSourceId = state.dataSourceId,
      requestToken = OAuth1RequestToken(event.requestToken, state.tokenSecret),
      verifier = event.tokenVerifier,
      params = event.params
    )
    accessParams.applicationData = state.applicationData
    accessParamsService.addParams(accessParams)
    eventBus.publish(this::class, OAuth1Event.AccessTokenAcquired(stateId = state.id, params = accessParams, dataSourceId = this.getId()))
  ...
  override fun registerAuthorizationRequestForUser(userId: String, dataSourceId: String, applicationData: String?): AuthorizationState {
    val requestToken: OAuth1RequestToken =
      authorizationOperator.acquireUnsignedRequestToken(getEstablishedAuthorizationRequestParams() as OAuth1AuthorizationRequestParams)
    val state = OAuth1AuthorizationState(userId, dataSourceId, requestToken.requestToken, requestToken.tokenSecret, applicationData)
    stateService.save(state)
    eventBus.publish(this::class, OAuth1Event.UserEnrollmentRequested(stateId = state.id, dataSourceId = this.getId()))
    eventBus.publish(this::class, OAuth1Event.UnauthorizedTokenAcquired(
      stateId = state.id, requestToken = requestToken.requestToken, tokenSecret = requestToken.tokenSecret, dataSourceId = this.getId()))
    return state
  }
  ...
  override fun getAuthorizationRedirectUri(state: AuthorizationState, params: AuthorizationRequestParams): String {
    ...
    return authorizationOperator.getCompleteAuthorizationUrlForUser(
      stateId = state.id,
      requestToken = OAuth1RequestToken(state.requestToken, state.tokenSecret),
      params = params
    )
  }
  ...
  override fun publishDataCollectionExecutionEvent(
    ...
    eventBus.publish(this::class, DataCollectionExecutionEvent.OAuth1ExecutionEvent(
  ...
  override fun getEstablishedAuthorizationRequestParams(): AuthorizationRequestParams {
    return OAuth1AuthorizationRequestParams()
  }
