...
import com.example.authenticationmodule.core.authorization.devices.dexcom.DexcomAccessTokenResponseExtractor
import com.example.authenticationmodule.core.authorization.devices.dexcom.DexcomDataSource
import com.example.authenticationmodule.core.authorization.devices.fitbit.FitbitAccessTokenResponseExtractor
import com.example.authenticationmodule.core.authorization.devices.fitbit.FitbitDataSource
import com.example.authenticationmodule.core.authorization.devices.withings.WithingsAccessTokenResponseExtractor
...
class OAuth2Operator(
    ...
    private val api: DefaultApi20 = OAuth2ApiDefinition(clientSettings.accessUri, clientSettings.authorizationUri)
    ...
    override fun retrieveAccessParams(
            ...
            webClient.postAbs("https://api.fitbit.com/1/user/-/apiSubscriptions/${userId}.json?subscriberId=1")
                ...
                .onSuccess { res ->
                    if (res.statusCode() >= 300) {
                        LOGGER.info("Subscription creation failed (${res.statusCode()}): ${res.bodyAsString()}")
                    } else {
        ...
        if (dataSourceId == WithingsDataSource.DATA_SOURCE_ID) {
            ...
            webClient.postAbs("https://wbsapi.withings.net/v2/signature")
                ...
                .addQueryParam("signature", nonceSignature)
                .send()
                .onSuccess { res ->
                    val bodyNode = ConfiguredObjectMapper.instance.readTree(res.bodyAsString())
                    ...
                    webClient.postAbs("https://wbsapi.withings.net/notify")
                        ...
                        .addQueryParam("callbackurl", subCallback)
                        .addQueryParam("appli", subAppli)
    ...
    private fun extractParams(userId: String, dataSourceId: String, rawResponse: String): OAuth2AccessParams {
        return when (dataSourceId) {
            ...
            DexcomDataSource.DATA_SOURCE_ID -> {
                DexcomAccessTokenResponseExtractor.getAccessParamsFromResponse(userId, dataSourceId, rawResponse)
            }
            else -> {
