...
class MainVerticle : AbstractVerticle() {
    ...
    private fun setUp(
        ...
        oauth2DataCollectionService =
            OAuth2DataCollectionService(eventBus, publisher, transformerRegistry, OAuth2DataCollectionOperatorBuilder(vertx, properties))
        ...
        vertx
            ...
            .listen { async ->
                if (async.succeeded()) {
                    ...
                    if (properties.getProperty("webserver.environment") == "local" ||
                        properties.getProperty("webserver.environment") == "test"
                    ) {
                        LOGGER.info(
                            "Authorization URI for Fitbit: https://localhost:$webServerPort/wearables/api/authorize/fitbit/userid?scopes=activity,heartrate,weight,sleep,nutrition,profile,settings&deploymentId=deploymentId",
                        )
                        LOGGER.info("Authorization URI for Garmin: https://localhost:$webServerPort/wearables/api/authorize/garmin/userid")
                        LOGGER.info(
                            "Authorization URI for Withings: https://localhost:$webServerPort/wearables/api/authorize/withings/userid?scopes=user.metrics,user.activitys&deploymentId=deploymentId",
                        )
                        LOGGER.info(
                            "Authorization URI for Dexcom: https://localhost:$webServerPort/wearables/api/authorize/dexcom/userid?deploymentId=deploymentId&scopes=",
                        )
